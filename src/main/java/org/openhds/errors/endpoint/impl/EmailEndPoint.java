...
public class EmailEndPoint implements ErrorServiceEndPoint {
    ...
    private void sendEmail(String type, ErrorLog errorLog) {
        ...
        if (type.equals(EmailEndPoint.CREATE)) {
            emailMessage.setText(MessageFormat.format(errorProperties.getCreateMessageBody(), marshalErrors(errorLog.getErrors()), errorLog.getCollectedBy().getFieldWorkerId(), errorLog.getUuid()));
            ...
            emailMessage.setText(MessageFormat.format(errorProperties.getResolveMessageBody(), errorLog.getUuid(), errorLog.getCollectedBy().getFieldWorkerId(), errorLog.getAssignedTo()));
